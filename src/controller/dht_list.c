...
int dht_list_get_nodes(struct sockaddr_in *sin, int num, struct sockaddr_in6 *sin6, int *num6)
{
...
int __dht_list_init(int port, int node_list_len, char **node_list)
{
    ...
    for (i = 0; i < node_list_len; i ++) {
        ...
        rc = getaddrinfo(node_list[i], node_list[i + 1], &hints, &info);
        if(rc != 0) {
